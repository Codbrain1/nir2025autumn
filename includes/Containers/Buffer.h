...
class Buffer
{
   ...
   public:
    Buffer(const std::string file_pos_name, const std::string file_conv_name) : buffer_posit(0), buffer_conv_laws(0)
    {
        if (file_pos_name.find(".txt") == std::string::npos || file_pos_name.find(".bin") == std::string::npos ||
            file_conv_name.find(".txt") == std::string::npos || file_conv_name.find(".bin") == std::string::npos) {
            std::runtime_error("The file name must not contain \".txt\" or \".bin\"");
        }
        if constexpr (type == Filetype::txt)  // проверка времени компиляции на текстовый или бинарный тип файла
        {
            file_positions.open(file_pos_name + ".txt", std::ios::app);
            file_conv_laws.open(file_conv_name + ".txt", std::ios::app);
        } else if constexpr (type == Filetype::bin) {
            file_positions.open(file_pos_name + ".bin", std::ios::app | std::ios::binary);
            file_conv_laws.open(file_conv_name + ".bin", std::ios::app | std::ios::binary);
        } else {
    ...
    void saveToFile_position()
    {
        for (int i = 0; i < buffer_posit.size(); ++i) {
            ...
            if constexpr (type == Filetype::txt) {
                ...
                file_positions << b.t << "\n" << b.N << "\n";
                for (int j = 0; j < b.N; ++j) {
                    file_positions << b.x[j] << "\t" << b.y[j] << "\t" << b.z[j] << "\t" << b.vx[j] << "\t" << b.vy[j] << b.vz[j]
                                   << "\n";
                }
            } else {
                auto& b = buffer_posit[i];
                ...
                for (int j = 0; j < b.N; ++j) {
                    T temp[6] = {b.x[j],  b.y[j],  b.z[j],
                                 b.vx[j], b.vy[j], b.vz[j]};  // запись данных позиций и скоростей в бинарный файл
...
#pragma omp critical(file_positions)
                {
                    ...
                    if constexpr (type == Filetype::txt) {
                        ...
                        file_positions << b.t << "\n" << b.N << "\n";
                        for (int j = 0; j < b.N; ++j) {
                            file_positions << b.x[j] << "\t" << b.y[j] << "\t" << b.z[j] << "\t" << b.vx[j] << "\t" << b.vy[j]
                                           << b.vz[j] << "\n";
                        }
                    } else {
                        file_positions.write(reinterpret_cast<char*>(&b.t), sizeof(T));  // запись момента времени в бинарный файл
